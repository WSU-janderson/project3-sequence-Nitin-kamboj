...
void testCopyConstructor(SequenceNode, ostream&);
void memoryLeakTest();
...
int main() {
...
#ifdef __GRADING
	ofstream OUTSTREAM;
	// change ./evals/ to where you want the evaluations saved
	...
	try {
		...
		data.insert(6, 777);
		OUTSTREAM << "Sequence:   " << data << endl;
		OUTSTREAM << "Should be:  <888, 100, 200, 300, 999, 400, 777, 500>"
			<< endl
			<< endl;
#else
		OUTSTREAM << "*** UNHANDLED CRASH DURING TESTING ***" << endl << endl;
	...
	try {
		...
		data.insert(5, 555);
		OUTSTREAM << "ERROR: insert() DID NOT throw an exception" << endl
			<< endl;
#else
		OUTSTREAM << "*** UNHANDLED CRASH DURING TESTING ***" << endl << endl;
#endif
	...
	try {
		...
		OUTSTREAM << "Empty sequence, empty returns: " << empty_sequence.empty()
			<< endl;
		OUTSTREAM << "Non-empty sequence, empty returns: "
			<< nonempty_sequence.empty() << endl
			<< endl;
#else
		OUTSTREAM << "*** UNHANDLED CRASH DURING TESTING ***" << endl << endl;
#endif
	...
	try {
		...
		OUTSTREAM << "Sequence length 7, size returned: " << data.size()
			<< endl;
		OUTSTREAM << "Empty sequence, size returned: " << empty_sequence.size()
			<< endl
			<< endl;
#else
		OUTSTREAM << "*** UNHANDLED CRASH DURING TESTING ***" << endl << endl;
	...
	try {
		...
		data.clear();
		OUTSTREAM << "Sequence cleared, empty returned: " << data.empty()
			<< endl;
